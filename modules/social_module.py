...
class SocialManager:
    ...
    def process_identity(self, user_id: str, current_nickname: str) -> bool:
        ...
        # 1. 노드 조회 (닉네임 업데이트 없이 가져오기 위해 빈 문자열 전달)
        # get_or_create_user(id, "")는 닉네임을 덮어쓰지 않고 노드만 리턴함
        node = self.graph.get_or_create_user(user_id, "")
        
        # 2. 신규 유저 처리 (닉네임이 비어있음)
        if not node.nickname:
            node.nickname = current_nickname
            return False # 첫 만남은 변경 이벤트가 아님

        # 3. 변경 감지 및 이력 기록
        if node.nickname != current_nickname:
            ...
            # (C) 로그 출력 (선택 사항)
            print(f"🔄 [Identity] Nickname Changed: {old_name} -> {current_nickname}")
