...
* boundary claim은 원문 대신 topic label + non-reversible fingerprint를 저장하고, 후속 턴 집행 정확도를 위해 정규화된 `sensitive_tokens`와 topic semantic terms를 함께 보존합니다. 제3자 claim은 `participants + audience_ids` ACL로 제한하며, legacy `shared` scope는 write/load 시 `participants`로 마이그레이션됩니다.
...
* boundary matching은 `sensitive_tokens`, topic label 기반 semantic terms, 임베딩 fallback을 함께 사용해, 완곡어/동의어 paraphrase가 assistant memory로 다시 들어오는 경로를 줄입니다.
