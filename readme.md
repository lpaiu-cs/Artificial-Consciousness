...
* boundary claim은 원문 대신 topic label + non-reversible fingerprint를 저장하고, 후속 턴 집행 정확도를 위해 정규화된 `sensitive_tokens`, topic semantic terms, target entity/alias fingerprint를 함께 보존합니다. 제3자 claim은 `participants + audience_ids` ACL로 제한하며, legacy `shared` scope는 write/load 시 `participants`로 마이그레이션됩니다.
...
* 암호화 키는 `CANONICAL_ENCRYPTION_KEY` env로 주입할 수 있고, 비어 있으면 기본적으로 SQLite DB 옆 sidecar key 파일을 생성해 재사용합니다.
...
* smoke suite는 pinned model gate, boundary payload 암호화, boundary enforcement 분기, semantic fallback budget/caching을 확인합니다.
...
4. boundary semantic fallback은 lexical/target match 이후에만 제한적으로 실행되며, rule embedding cache + segment/result cache로 반복 비용을 억제합니다.
