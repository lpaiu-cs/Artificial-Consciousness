...
* **Entity-aware Query Planning:** `QueryPlanner`는 이름 직접 매칭에 더해 **세션 referent cache + 관계 role alias**를 사용해 `"걔"`, `"그 교수님"` 같은 후속 참조를 해석하고, 못 풀면 user 기본값으로 추정하지 않고 `uncertainties`로 넘깁니다.
...
* **Fulfillment-aware Reliability:** assistant promise / follow-up 성격의 `open_loop`는 overdue / done / abandoned lifecycle 이벤트를 SocialManager로 보내며, `reliability`는 말투보다 실제 이행 결과에 의해 보정됩니다.
...
* boundary claim은 원문 대신 topic label + non-reversible fingerprint를 저장하고, 후속 턴 집행 정확도를 위해 정규화된 `sensitive_tokens`, topic semantic terms, target entity/alias fingerprint를 canonical payload에 함께 보존합니다. 제3자 claim은 `participants + audience_ids` ACL로 제한하며, legacy `shared` scope는 write/load 시 `participants`로 마이그레이션됩니다.
* high-sensitivity `boundary.rule` payload는 SQLite 평문 JSON에 그대로 남기지 않습니다. 현재 구현은 public `value_json/qualifiers_json`에는 topic label 같은 최소 필드만 남기고, `sensitive_tokens`, semantic terms, target scope payload는 애플리케이션 레벨 암호화 blob으로 분리해 저장합니다.
* `ltm_graph.json`과 delta log는 boundary full payload를 저장하지 않습니다. graph persistence에는 `kind`, `topic_label`, ACL 같은 public projection만 남고, `target_entity_id`, `sensitive_tokens`, `semantic_terms`, target alias/role 세부값은 canonical encrypted blob 쪽에만 남습니다.
* boundary redaction은 턴 전체를 통째로 placeholder로 바꾸지 않고, boundary가 걸린 절만 치환해 같은 턴의 무해한 interaction preference나 open-loop 신호는 계속 추출합니다.
* 활성 `boundary.rule` claim은 매 턴 assistant self-memory redaction에 다시 합쳐집니다. 즉 현재 턴 경계뿐 아니라 과거에 저장된 장기 경계도 후속 턴의 memory-safe text 집행에 참여합니다.
* boundary matching은 `sensitive_tokens`, topic label 기반 semantic terms, target alias/role fingerprint, 임베딩 fallback을 함께 사용해, 특정 대상 경계와 완곡어/동의어 paraphrase가 assistant memory로 다시 들어오는 경로를 줄입니다.
* semantic fallback은 모든 boundary에 대해 무제한으로 돌지 않습니다. 현재 구현은 lexical/target match를 먼저 사용하고, 임베딩 비교는 상한이 있는 candidate budget과 result/segment cache 뒤에서만 동작시켜 비용과 지연을 제어합니다.
* `do_not_store_sensitive`와 `avoid_topic`는 동일하게 취급하지 않습니다. 전자는 memory redaction 중심으로 집행되고, 후자만 user-visible response repair까지 강하게 적용됩니다.
* 활성 boundary policy는 post-generation repair만 하는 것이 아니라 generation prompt에도 매 턴 주입됩니다. 현재 repair 단계는 마지막 safety net 역할을 합니다.
* 암호화 키는 keyring으로 관리됩니다. `CANONICAL_ENCRYPTION_KEYS_JSON` 또는 `CANONICAL_ENCRYPTION_KEYRING_PATH`로 여러 키를 제공할 수 있고, `CANONICAL_ACTIVE_KEY_ID`가 현재 암호화 키를 결정합니다.
* legacy `CANONICAL_ENCRYPTION_KEY` 한 개만 제공되면 `{CANONICAL_ENCRYPTION_KEY_ID: CANONICAL_ENCRYPTION_KEY}` keyring으로 간주합니다. 아무 env key도 없으면 SQLite DB 옆에 `{"active_key_id":"...","keys":{"...":"..."}}` 형식의 sidecar keyring 파일을 생성해 재사용합니다.
* `CanonicalMemoryStore.reencrypt_protected_claim_payloads()`로 boundary encrypted payload를 새 active key로 재암호화할 수 있으며, `payload_key_id`는 실제 복호화 key selector로 사용됩니다.
* boundary reward는 “활성 경계가 존재한다”는 이유만으로 붙지 않습니다. 현재 턴이 실제로 boundary-relevant했고 assistant가 그 조건을 어기지 않았을 때만 `boundary_respected`가 관계 신호로 반영됩니다.
* 반대로 raw 응답이 금지 주제를 다시 꺼내면 `boundary_violated` 음수 신호가 관계 상태에 반영되고, 사용자에게 보이는 응답도 반환 직전에 boundary-safe placeholder로 자동 repair 됩니다.
...
MODEL_EVAL_GATE_ENABLED = True
MODEL_EVAL_GATE_PATH = "model_eval_gate.json"
CANONICAL_ACTIVE_KEY_ID = os.getenv("CANONICAL_ACTIVE_KEY_ID", "local-v1")
CANONICAL_ENCRYPTION_KEYS_JSON = os.getenv("CANONICAL_ENCRYPTION_KEYS_JSON", "")
CANONICAL_ENCRYPTION_KEYRING_PATH = os.getenv("CANONICAL_ENCRYPTION_KEYRING_PATH", "")
CANONICAL_AUTO_REENCRYPT_PROTECTED_PAYLOADS = False
...
3. `commitment.open_loop`는 canonical store에서 overdue / close lifecycle 이벤트를 발생시키고, `assistant_commitment` 또는 assistant-owned loop만 `reliability`와 `trust` 갱신 대상으로 취급합니다.
4. retrieval 질문의 대상 entity는 이름 substring만 보지 않고, 세션 referent cache와 relation role alias를 같이 써서 후속 지시어를 보수적으로 해석합니다.
5. `BotOrchestrator`는 `current_mood` 전역값 대신 **user-scoped session mood**를 retrieval과 generation에 전달합니다.
6. 결과적으로 한 사용자의 분위기가 다른 사용자에게 번지는 문제를 줄이고, 관계 상태도 단일 similarity 복제에서 벗어나도록 정리했습니다.

### 5. Reproducibility and Drift Control

현재 구현은 운영 드리프트와 검증 재현성을 분리해서 다룹니다.
...
* **Richer Coreference Resolution:** referent cache heuristic을 span-level mention resolution, role memory, unresolved referent abstention으로 확장.
