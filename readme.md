...
* boundary redaction은 이제 턴 전체를 통째로 placeholder로 바꾸지 않고, boundary가 걸린 절만 치환해 같은 턴의 무해한 interaction preference나 open-loop 신호는 계속 추출합니다.
...
3. `commitment.open_loop`는 canonical store에서 overdue / close lifecycle 이벤트를 발생시키고, assistant 책임 loop의 `done` / `abandoned` 결과가 `reliability`와 `trust`에 직접 반영됩니다.
