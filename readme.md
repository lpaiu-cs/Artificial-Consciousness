...
* **Durable Cursor:** delta ingest cursor를 파일에 영속화하고 최대 개수를 제한해, 장기 실행과 재시작 이후에도 중복 ingest를 줄입니다.
...
* boundary claim은 원문 대신 topic label + non-reversible fingerprint만 저장하고, 제3자 claim은 `participants + audience_ids` ACL로 제한합니다.
...
4. participant-shared claim은 `audience_ids`가 있는 경우에만 canonical/graph retrieval에서 노출됩니다.
