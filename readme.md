...
* 관계는 여전히 벡터 기반 `affinity`를 사용하지만, 갱신 신호는 봇이 스스로 만든 감정 문구보다 **사용자의 실제 발화**를 우선 반영합니다.
* 장기적으로는 `trust / warmth / familiarity / respect / tension / reliability` 같은 다축 관계 상태로 확장할 계획이지만, 현재 코드는 그 전 단계로서 과도한 자기감정 반영을 줄이는 방향으로 정리되었습니다.
...
* **Vector Social Logic:** 관계 갱신은 답변의 자기 감정보다 **사용자 입력의 벡터 신호**를 사용해 affinity를 조정합니다.
...
| **LTM Graph** | 노드 및 엣지 관리 | **Append-only Log** (JSON Snapshot + Delta Replay) | 현재 구현은 Graph 단일 저장소, SQLite canonical store는 다음 단계 |
...
* **Entity Node (인물):** 유저의 정체성, 닉네임 이력, affinity.
...
현재 사회성 모델은 아직 단일 축 `affinity`를 사용합니다.

1. `SocialManager`는 긍정 기준점 임베딩을 유지합니다.
2. `BotOrchestrator`는 **assistant의 자기 감정 문구가 아니라 사용자 입력**을 벡터화해 관계 신호로 사용합니다.
3. 결과적으로 자기서사에 과적합되는 관계 갱신을 줄이고, 실제 상호작용 입력을 더 강하게 반영합니다.
...
* **SQLite Canonical Store:** `Claim`과 `Open Loop`를 그래프에서 분리해 상태 조회와 supersede/expiry를 더 안정적으로 처리.
* **Multi-axis Relation State:** `affinity` 단일 점수에서 `trust / warmth / familiarity / respect / tension / reliability` 구조로 이동.
* **Structured Outputs:** reflection JSON mode를 JSON Schema 기반 structured outputs로 치환.
