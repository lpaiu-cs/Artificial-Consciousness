...
* boundary matching은 현재 sensitive token 외에도 topic label 기반 semantic term과 임베딩 fallback을 함께 사용해, 완곡어/동의어 paraphrase가 assistant memory로 다시 들어오는 경로를 줄입니다.
* boundary respect는 더 이상 “경계를 요청했다”는 사실만으로 가산되지 않습니다. 응답 생성 후 실제 assistant self-memory/reflector 경로에 금지 주제가 다시 남지 않았을 때만 관계 신호로 반영합니다.
