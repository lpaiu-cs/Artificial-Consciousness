...
from cogbot.memory.ontology import Facet, FACET_SPECS, get_facet_spec
...
class ReflectionHandler:
    ...
    def _analyze_with_llm(self, memories: List[MemoryObject]) -> Dict[str, Any]:
        ...
        3. "claims": A list of canonical state updates. Each item must include:
           ...
           - "value_fields": list of field entries for the claim value object
           - "qualifier_fields": list of field entries for the qualifiers object
        ...
        - For strict JSON compliance, encode claim payload objects as field lists:
          "value_fields": [{{"key": "dimension", "value_json": "language"}}, {{"key": "value", "value_json": "ko"}}]
          "qualifier_fields": [{{"key": "audience_ids", "value_json": "[\\"user_001\\", \\"bot\\"]"}}]
        - In each field entry, "value_json" must be a string. Use JSON literals for arrays, objects, booleans, nulls, and numbers.
        """
                    ...
                    "claims": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "facet": {"type": "string"},
                                "value_fields": self._reflection_field_array_schema(),
                                "qualifier_fields": self._reflection_field_array_schema(),
                                "source_type": {
                                    "type": "string",
    ...
    def _reflection_field_array_schema(self) -> Dict[str, Any]:
        return {
            "type": "array",
            "items": {
                "type": "object",
    ...
    def _normalize_analysis_result(self, response: Dict[str, Any], memories: List[MemoryObject]) -> Dict[str, Any]:
        ...
        for claim in response.get("claims", []):
            ...
            value = self._decode_claim_payload_object(
                claim.get("value_fields") if "value_fields" in claim else claim.get("value")
            )
            qualifiers = self._decode_claim_payload_object(
                claim.get("qualifier_fields") if "qualifier_fields" in claim else claim.get("qualifiers")
            )
    ...
    def _decode_claim_payload_object(self, payload: Any) -> Dict[str, Any]:
        ...
        for entry in payload:
            ...
            if not key:
                continue
            decoded[key] = self._decode_claim_payload_value(entry.get("value_json"))
        return decoded

    def _decode_claim_payload_value(self, raw_value: Any) -> Any:
